...
class quiz_page : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onClick(view: View) {
        ...
        if(clickedButton.id == R.id.final_Btn){
            ...
            sharedPreferences.edit().putString("pretest_score",score.toString()).apply()
            finishQuiz()
            return
        }
...
//                ansF.visibility = View.VISIBLE
//            }
            editor.putBoolean("submitButtonClicked", true)
            editor.apply()
        } else {
    ...
    private fun finishQuiz() {
        ...
        val alertDialog = AlertDialog.Builder(this)
            .setTitle(passStatus)
            .setMessage("Score is $score out of $totalQuestion answers are $answer")
            .setCancelable(false)
            .show()
        ...
        Handler().postDelayed({
            ...
            val intent = Intent(this@quiz_page, Drawer::class.java)
            intent.putExtra("score", score.toString())
            startActivity(intent)
            finish()
        }, 5000) // 5000 milliseconds = 5 seconds
        val retrofit = Retrofit.Builder()
            .baseUrl("http://165.22.212.47/api/score_pretest/")
            .addConverterFactory(GsonConverterFactory.create())
            .build()
        var scoreApi = retrofit.create(myapi_score::class.java)
...
//                        sharedPreferences.edit().putLong(Drawer.KEY_PRETIME, time).apply()
                        val time_preference = getSharedPreferences("QuizPreferences", Context.MODE_PRIVATE)
                        time_preference.edit().putLong(Drawer.KEY_PRETIME, time).apply()
//                        showToast(time.toString())
//                        sharedPreferences.edit().putBoolean(Drawer.KEY_LOCK_PRE, true).apply()
                    }
                    else{
